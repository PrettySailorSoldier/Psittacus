...
fn main() {
    ...
    //
    // Instead, we copy directly from `src-tauri/binaries/` — where the files
    // already carry the full triple-suffix name — into the profile output dir
    // with that same name.  `CARGO_MANIFEST_DIR` always points at `src-tauri/`
    // and is set before build.rs runs, so the source is always available.
    // -----------------------------------------------------------------------
